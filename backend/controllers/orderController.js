...
const getAcceptedOrdersByEmail = async (req, res) => {
  ...
  try {
    const acceptedOrders = await Order.find({ userEmail: userEmail, status: "accepted" });
    ...
    console.error(error);
    return res.status(500).json({ error: "An error occurred while fetching accepted orders." });
  }
};
...
const getRejectedOrdersByEmail = async (req, res) => {
  ...
  try {
    const rejectedOrders = await Order.find({ userEmail: userEmail, status: "rejected" });
    ...
    console.error(error);
    return res.status(500).json({ error: "An error occurred while fetching rejected orders." });
  }
};
...
const getCompletedOrdersByEmail = async (req, res) => {
  ...
  try {
    const completedOrders = await Order.find({ userEmail: userEmail, status: "completed" });
    ...
    console.error(error);
    return res.status(500).json({ error: "An error occurred while fetching completed orders." });
  }
};
