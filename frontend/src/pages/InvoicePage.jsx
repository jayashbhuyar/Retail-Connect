...
import { ChevronDown, ChevronUp, Calendar, Package, DollarSign, SortDesc } from "lucide-react";
...
const InvoicePage = () => {
  ...
  const renderInvoiceDetails = () => {
    ...
    return (
      ...
      <div id="invoice-content" className="p-4 md:p-8 bg-gradient-to-r from-indigo-50 to-purple-50">
        <div className="mb-8 text-center">
          <h1 className="text-3xl md:text-4xl font-bold text-indigo-700">Invoice</h1>
          <p className="text-lg md:text-xl text-gray-600">#{selectedInvoice.invoiceNo}</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-8">
          {renderInfoCard("Order Date", formatDate(selectedInvoice.createdAt), "bg-blue-100")}
          {renderInfoCard("Acceptance Date", selectedInvoice.orderStatus === "accepted" ? formatDate(selectedInvoice.updatedAt) : "Pending", "bg-green-100")}
          {renderInfoCard("Delivery Before", formatDate(selectedInvoice.deliveryBefore), "bg-yellow-100")}
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-8">
          {renderContactCard("Distributor", {
        ...
        <div className="mb-8 bg-white p-4 md:p-6 rounded-lg shadow-lg border border-indigo-200">
          <h3 className="text-xl md:text-2xl font-semibold mb-4 text-indigo-700 border-b pb-2">Product Details</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="col-span-1 md:col-span-2">
                ...
                <table className="w-full text-left">
                  ...
                  <tbody>
                    {selectedInvoice.product ? (
                      <tr>
                        <td className="p-2 border-b">{selectedInvoice.product.productName}</td>
                        <td className="p-2 border-b">{selectedInvoice.product.quantity}</td>
                        <td className="p-2 border-b">${selectedInvoice.product.price.toFixed(2)}</td>
                        <td className="p-2 border-b">${selectedInvoice.product.total.toFixed(2)}</td>
                      </tr>
                    ) : (
                      <tr>
                        <td colSpan="4" className="p-2 border-b text-center">No product information available</td>
                      </tr>
                    )}
                  </tbody>
                </table>
              ...
              <div className="mt-4">
                <h4 className="font-semibold text-indigo-600">Product Description</h4>
                <p className="text-gray-700">{selectedInvoice.product?.productDescription || 'No description available'}</p>
              </div>
            </div>
            <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-8">
          <div className="bg-white p-4 md:p-6 rounded-lg shadow-lg border border-indigo-200">
            <h3 className="text-xl md:text-2xl font-semibold mb-4 text-indigo-700 border-b pb-2">Payment QR Code</h3>
            <div className="flex justify-center">
              <img
                src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=https://payment.example.com/${selectedInvoice.invoiceNo}`}
                alt="Payment QR Code"
                className="w-36 h-36 md:w-48 md:h-48"
              />
            </div>
            <p className="text-center mt-4 text-sm text-gray-500">Scan to make payment</p>
          </div>
          <div className="bg-white p-4 md:p-6 rounded-lg shadow-lg border border-indigo-200">
            <h3 className="text-xl md:text-2xl font-semibold mb-4 text-indigo-700 border-b pb-2">Order Summary</h3>
            <div className="space-y-2">
              <div className="flex justify-between">
                <span>Status:</span>
                <span className="font-semibold">{selectedInvoice.orderStatus}</span>
              </div>
              <div className="flex justify-between">
                <span>Net Amount:</span>
                <span className="font-semibold">${selectedInvoice.netAmount.toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span>Authorized Signature:</span>
                <span className="font-semibold">{selectedInvoice.authorizedSignature}</span>
              </div>
            </div>
          </div>
        </div>
